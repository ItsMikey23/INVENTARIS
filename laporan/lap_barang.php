...
$pdf->Cell(26.3,0.7,"Laporan Kategori Barang",0,10,'C');
$pdf->ln(0.2);
$pdf->SetFont('Arial','I',10);
$pdf->Cell(5,0.7,"Di cetak pada : ".date("D-d/Y"),0,0,'C');
$pdf->ln(1);
$pdf->SetFont('Arial','i',10);
$pdf->Cell(1, 0.7, 'No', 1, 0, 'C');
$pdf->Cell(3, 0.7, 'Kode Barang', 1, 0, 'C');
$pdf->Cell(14.5, 0.7, 'Nama Barang', 1, 0, 'C');
$pdf->Cell(6, 0.7, 'Spesifikasi', 1, 0, 'C');
$pdf->Cell(3, 0.7, 'Kategori', 1, 0, 'C');
$pdf->ln(0.7);
$pdf->SetFont('Arial','i',10);
$query = mysqli_query($conn,"SELECT * FROM tbl_barang ORDER BY kode_barang ASC");
$no=1;
while ($lihat = mysqli_fetch_array($query)) { 
	$pdf->Cell(1, 0.7, $no , 1, 0, 'C');
	$pdf->Cell(3, 0.7, $lihat['kode_barang'],1, 0, 'C');
	$pdf->Cell(14.5, 0.7, $lihat['nama_barang'],1, 0, 'C');
	$pdf->Cell(6, 0.7, $lihat['spesifikasi'],1, 0, 'C');
	$pdf->Cell(3, 0.7, $lihat['kategori'], 1, 1,'C');
	$no++;
}
