...
$pdf->Cell(26.3,0.7,"Laporan Barang Rusak",0,10,'C');
$pdf->ln(0.2);
...
$pdf->Cell(5, 0.7, 'Lokasi', 1, 0, 'C');
$pdf->Cell(2.7, 0.7, 'Jenis Rusak', 1, 0, 'C');
$pdf->Cell(5.5, 0.7, 'Keterangan', 1, 0, 'C');
$pdf->ln(0.7);
$pdf->SetFont('Arial','i',10);

if(isset($_GET['laporan1'])) {
	$tahun=$_GET['tahun'];
	$bulan=$_GET['bulan'];
	$query = mysqli_query($conn,"SELECT * FROM tbl_rusakbarang WHERE tgl_rusak LIKE '$tahun%' AND tgl_rusak LIKE '_____$bulan%' ORDER BY tgl_rusak ASC");
}
else{
	$query = mysqli_query($conn,"SELECT * FROM tbl_rusakbarang ORDER BY id_barangrusak ASC");
}

$no=1;
while ($lihat = mysqli_fetch_array($query)) { 
	$pdf->Cell(1, 0.7, $no , 1, 0, 'C');
    $pdf->Cell(2, 0.7, $lihat['id_barangrusak'],1, 0, 'C');
	$pdf->Cell(6, 0.7, $lihat['nama_barang'],1, 0, 'C');
	$pdf->Cell(2.5, 0.7, $lihat['tgl_rusak'],1, 0, 'C');
	$pdf->Cell(1.5, 0.7, $lihat['jumlah_rusak'], 1, 0,'C');
	$pdf->Cell(5, 0.7, $lihat['lokasi_barang'],1, 0, 'C');
	$pdf->Cell(2.7, 0.7, $lihat['kondisi'],1, 0, 'C');
